...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    Promise.all([api.getSoldiers(), api.getSections()])
      .then(([soldiers, sections]) => {
        const s = soldiers.find(x => x.id === currentUser.id);
        if (!s) return;
        setSoldier(s);
        ...
        setForm({
          ...
          idNumber: s.idNumber ?? '', personalNumber: s.personalNumber ?? '',
        });
      })
      .catch(() => showToast('שגיאה בטעינת הפרופיל'));
  }, [currentUser]);
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const updated = await api.updateSoldier(soldier.id, { ...soldier, ...form });
      setSoldier(updated);
      setEditing(false);
          ...
          <form onSubmit={handleSave} className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 space-y-3">
            ...
            <div className="flex gap-3 pt-1">
              <button type="submit" disabled={saving}
                className="flex-1 bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white py-2 rounded-lg font-medium transition-colors">
                {saving ? 'שומר...' : 'שמור'}
              </button>
