...
import { fullName, canAccessSoldierManagement, canAccessTaskManagement } from "@/lib/permissions";
...
const ALL_ITEMS = [
  { href: "/my-tasks",  label: "המשימות שלי",   icon: "📌", color: "bg-orange-500", canAccess: (_: Soldier) => true },
  { href: "/phonebook", label: "ספר טלפונים",   icon: "📞", color: "bg-teal-500",   canAccess: (_: Soldier) => true },
  { href: "/schedule",  label: 'שבצ"ק יומי',    icon: "📅", color: "bg-purple-600", canAccess: (_: Soldier) => true },
  { href: "/soldiers",  label: "ניהול כוח אדם", icon: "👥", color: "bg-blue-600",   canAccess: canAccessSoldierManagement },
  { href: "/tasks",     label: "ניהול משימות",  icon: "📋", color: "bg-green-600",  canAccess: canAccessTaskManagement },
];
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    api.getSections().then(sections => {
      const sec = sections.find(s => s.id === currentUser.sectionId);
  ...
  const visibleItems = ALL_ITEMS.filter(item => item.canAccess(currentUser));
